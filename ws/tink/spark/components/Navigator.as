...
//	[IconFile("DataGroup.png")]
	 ...
	 *  @productversion Flex 4
	 */
	public class Navigator extends GroupBase implements ISelectableList
	{
		/**
		 *  Constructor.
		 *  
...
//			setUpItemRenderer(renderer, 0, _typicalItem);
		...
		{
			if ((index < 0) || (_mxmlContent == null) || (index >= _mxmlContent.length))
				return null;
			
			const item:UIComponent = _mxmlContent[ index ];
...
//			const list:IList = dataProvider;
			if (_mxmlContent && (_mxmlContent.length > 0))
			{
				_typicalItem = _mxmlContent[0];
...
//				changeUseVirtualLayout();
			...
			if (value)
			{
				...
				if( value.useVirtualLayout != _useVirtualLayout )
				{
					changeUseVirtualLayout();
...
//			}
		...
		public function getItemIndicesInView():Vector.<int>
		{
			...
			const dataProviderLength:int = _mxmlContent.length;
...
//			}
		...
		private function createItemRenderers():void
		{
			...
			{
//				var item:UIComponent = _mxmlContent[ index ];
...
//				{
					removeRendererAt(index); 
					itemAdded( _mxmlContent[ index ], index);
//				}
			}
		...
		{ 
			...
			if ( useVirtualLayoutChanged || _mxmlContentChanged)
			{
				...
				if (_mxmlContentChanged)
				{
					_mxmlContentChanged = false;
...
//			const recyclingOK:Boolean = isRecyclingOK();       
			...
			for each (var vrIndex:int in oldVirtualRendererIndices)
			{
				...
				var item:UIComponent = (_mxmlContent.length > vrIndex) ? _mxmlContent[ vrIndex ] : null;
...
//					setUpItemRenderer(elt, index, item);
		...
		{
			if ((_mxmlContent == null) || (element == null))
				return -1;
...
//			setUpItemRenderer(myItemRenderer, index, item);
		...
		{ 
			...
			if (childParent is Navigator)
				Navigator(childParent)._removeChild(child);
...
//			resetRenderersIndices();
		...
		override protected function createChildren():void
		{
			...
			layout.useVirtualLayout = useVirtualLayoutChanged;
		...
		{
			if( !_mxmlContent ) mxmlContent = new Vector.<UIComponent>();
//			dp.addItemAt( item, index );
			_mxmlContent.splice( index, 0, item );
			
			_mxmlContentChanged = true;
			invalidateDisplayList();
			invalidateProperties();
		}
		...
		public function removeAll():void
		{
			...
			_mxmlContent = new Vector.<UIComponent>();
		...
		{
			if( !_mxmlContent || index >= numElements ) return null;
			var item:UIComponent = _mxmlContent.splice( index, 1 )[ 0 ];
			_mxmlContentChanged = true;
			invalidateDisplayList();
			invalidateProperties();
		...
		{
			if( !_mxmlContent || index > numElements ) return null;
			_mxmlContent[ index ] = UIComponent( item );
			_mxmlContentChanged = true;
			invalidateDisplayList();
			invalidateProperties();
			return item;
		...
		{
			if( !_mxmlContent ) return null;
			
			var a:Array = new Array();
			for each (var value:UIComponent in _mxmlContent )
			{
				a.push( value );
			}
		...
		{
			if( _useVirtualLayout == value ) return;
		 ...
		 *  @productversion Flex 4
		 */
		private var _mxmlContentChanged	: Boolean;
		protected var _mxmlContent	: Vector.<UIComponent>
		public function set mxmlContent(value:Vector.<UIComponent>):void
		{
			if( _mxmlContent == value ) return;
		 ...
		 *  @private
		 */
		mx_internal function getMXMLContent():Vector.<UIComponent>
		{
			if (_mxmlContent)
