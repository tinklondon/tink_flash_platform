...
//	include "../styles/metadata/BasicInheritingTextStyles.as"
	 ...
	 *  @productversion Flex 4
	 */
	public class SkinnableDataNavigator extends SkinnableContainerBase implements IItemRendererOwner, ISelectableList
	{
//			include "../core/Version.as";
		 ...
		 *  @productversion Flex 4
		 */
		public function SkinnableDataNavigator()
		{
			super();
		}
		 ...
		 *  @productversion Flex 4
		 */
		public var dataNavigator:DataNavigator;
		
		/**
		 *  @private
		 *  Several properties are proxied to dataNavigator.  However, when dataNavigator
		 *  is not around, we need to store values set on SkinnableDataContainer.  This object 
		 *  stores those values.  If dataNavigator is around, the values are stored 
		 *  on the dataNavigator directly.  However, we need to know what values 
		 *  have been set by the developer on the SkinnableDataContainer (versus set on 
		 *  the dataNavigator or defaults of the dataNavigator) as those are values 
		 *  we want to carry around if the dataNavigator changes (via a new skin). 
		 ...
		 *  dataNavigator is not around, dataGroupProperties is a typeless 
		 *  object to store these proxied properties.  When dataNavigator is around,
		...
		//--------------------------------------------------------------------------
		//
		//  Properties proxied to dataNavigator
		//
		//--------------------------------------------------------------------------
		...
		{
			if (dataNavigator)
				return dataNavigator.autoLayout;
			else
			{
		...
		public function set autoLayout(value:Boolean):void
		{
			if (dataNavigator)
			{
				dataNavigator.autoLayout = value;
		...
		public function set dataProvider(value:IList):void
		{
			if (dataNavigator)
			{
				dataNavigator.dataProvider = value;
				dataGroupProperties = BitFlagUtil.update(dataGroupProperties as uint, 
		...
		public function set itemRenderer(value:IFactory):void
		{
			if (dataNavigator)
			{
				dataNavigator.itemRenderer = value;
				dataGroupProperties = BitFlagUtil.update(dataGroupProperties as uint, 
		...
		{
			return (dataNavigator) 
			? dataNavigator.itemRendererFunction 
		...
		public function set itemRendererFunction(value:Function):void
		{
			if (dataNavigator)
			{
				dataNavigator.itemRendererFunction = value;
				dataGroupProperties = BitFlagUtil.update(dataGroupProperties as uint, 
		...
		public function set typicalItem(value:Object):void
		{
			if (dataNavigator)
			{
				dataNavigator.typicalItem = value;
				dataGroupProperties = BitFlagUtil.update(dataGroupProperties as uint, 
		...
		override protected function partAdded(partName:String, instance:Object):void
		{
			...
			{
				// copy proxied values from dataGroupProperties (if set) to dataNavigator
				
				var newDataGroupProperties:uint = 0;
				
				if (dataGroupProperties.layout !== undefined)
				{
					dataNavigator.layout = dataGroupProperties.layout;
				...
				{
					dataNavigator.autoLayout = dataGroupProperties.autoLayout;
					newDataGroupProperties = BitFlagUtil.update(newDataGroupProperties as uint, 
						AUTO_LAYOUT_PROPERTY_FLAG, true);
				}
				...
				{
					dataNavigator.dataProvider = dataGroupProperties.dataProvider;
					newDataGroupProperties = BitFlagUtil.update(newDataGroupProperties as uint, 
						DATA_PROVIDER_PROPERTY_FLAG, true);
				}
				...
				{
					dataNavigator.itemRenderer = dataGroupProperties.itemRenderer;
					newDataGroupProperties = BitFlagUtil.update(newDataGroupProperties as uint, 
						ITEM_RENDERER_PROPERTY_FLAG, true);
				}
				...
				{
					dataNavigator.itemRendererFunction = dataGroupProperties.itemRendererFunction;
					newDataGroupProperties = BitFlagUtil.update(newDataGroupProperties as uint, 
						ITEM_RENDERER_FUNCTION_PROPERTY_FLAG, true);
				}
				...
				{
					dataNavigator.typicalItem = dataGroupProperties.typicalItem;
					newDataGroupProperties = BitFlagUtil.update(newDataGroupProperties as uint, 
						TYPICAL_ITEM_PROPERTY_FLAG, true);
				}
				...
				// Register our instance as the dataNavigator's item renderer update delegate.
				dataNavigator.rendererUpdateDelegate = this;
				...
				{
					dataNavigator.addEventListener(
		...
		override protected function partRemoved(partName:String, instance:Object):void
		{
			...
			if (instance == dataNavigator)
			{
				dataNavigator.removeEventListener(
					RendererExistenceEvent.RENDERER_ADD, dispatchEvent);
				dataNavigator.removeEventListener(
					RendererExistenceEvent.RENDERER_REMOVE, dispatchEvent);
				
				// copy proxied values from dataNavigator (if explicitly set) to dataGroupProperties
				
				var newDataGroupProperties:Object = {};
				
				if (BitFlagUtil.isSet(dataGroupProperties as uint, LAYOUT_PROPERTY_FLAG))
					newDataGroupProperties.layout = dataNavigator.layout;
				
				if (BitFlagUtil.isSet(dataGroupProperties as uint, AUTO_LAYOUT_PROPERTY_FLAG))
					newDataGroupProperties.autoLayout = dataNavigator.autoLayout;
				
				if (BitFlagUtil.isSet(dataGroupProperties as uint, DATA_PROVIDER_PROPERTY_FLAG))
					newDataGroupProperties.dataProvider = dataNavigator.dataProvider;
				
				if (BitFlagUtil.isSet(dataGroupProperties as uint, ITEM_RENDERER_PROPERTY_FLAG))
					newDataGroupProperties.itemRenderer = dataNavigator.itemRenderer;
				
				if (BitFlagUtil.isSet(dataGroupProperties as uint, ITEM_RENDERER_FUNCTION_PROPERTY_FLAG))
					newDataGroupProperties.itemRendererFunction = dataNavigator.itemRendererFunction;
				
				if (BitFlagUtil.isSet(dataGroupProperties as uint, TYPICAL_ITEM_PROPERTY_FLAG))
					newDataGroupProperties.typicalItem = dataNavigator.typicalItem;
				
				dataGroupProperties = newDataGroupProperties;
				
				dataNavigator.dataProvider = null;
				dataNavigator.layout = null;
				dataNavigator.rendererUpdateDelegate = null;
			}
		}
		 ...
		 *  listen for them on our dataNavigator.
		 */
			...
			type:String, listener:Function, useCapture:Boolean=false, priority:int=0, useWeakReference:Boolean=false) : void
		{
			...
			// if it's a different type of event or the dataNavigator doesn't
			// exist, don't worry about it.  When the dataNavigator, 
			// gets created up, we'll check to see whether we need to add this 
			// event listener to the dataNavigator.
			
			if (type == RendererExistenceEvent.RENDERER_ADD && dataNavigator)
			{
				dataNavigator.addEventListener(
					RendererExistenceEvent.RENDERER_ADD, dispatchEvent);
			}
			
			if (type == RendererExistenceEvent.RENDERER_REMOVE && dataNavigator)
			{
				dataNavigator.addEventListener(
					RendererExistenceEvent.RENDERER_REMOVE, dispatchEvent);
			}
		...
		override public function removeEventListener(type:String, listener:Function, useCapture:Boolean=false) : void
		{
			...
			// remove our underlying event listener from the dataNavigator.
			if (type == RendererExistenceEvent.RENDERER_ADD && dataNavigator)
			{
				if (!hasEventListener(RendererExistenceEvent.RENDERER_ADD))
				{
