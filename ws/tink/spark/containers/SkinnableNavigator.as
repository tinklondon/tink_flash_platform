...
//	[IconFile("SkinnableContainer.png")]
	 ...
	 *  @productversion Flex 4
	 */
	public class SkinnableNavigator extends SkinnableContainerBase 
...
//		include "../core/Version.as";
		 ...
		 *  @productversion Flex 4
		 */
		public function SkinnableNavigator()
		{
			super();
		}
		...
		private var _placeHolderGroup:Navigator;
		
		mx_internal function get currentNavigator():Navigator
		{          
			...
			if (!navigator)
			{
				...
				{
					_placeHolderGroup = new Navigator();
		...
		//--------------------------------------------------------------------------
		//
		//  Properties proxied to navigator
		//
		//--------------------------------------------------------------------------
		...
		/**
		 *  @private
		 *  Variable used to store the mxmlContent when the navigator is 
		 ...
		 *  that is in the navigator of a skin.
		 */
			...
			if (instance == navigator)
			{
				if (_contentModified)
				{
					if (_placeHolderGroup != null)
					{
						...
						navigator.mxmlContent = sourceContent ? sourceContent.slice() : null;
						
					}
					else if (_mxmlContent != null)
					{
						navigator.mxmlContent = _mxmlContent;
						_mxmlContent = null;
					}
				}
				
				// copy proxied values from contentGroupProperties (if set) to navigator
				
				var newContentGroupProperties:uint = 0;
				
				if (contentGroupProperties.autoLayout !== undefined)
				{
					navigator.autoLayout = contentGroupProperties.autoLayout;
				...
				{
					navigator.layout = contentGroupProperties.layout;
					newContentGroupProperties = BitFlagUtil.update(newContentGroupProperties, 
						LAYOUT_PROPERTY_FLAG, true);
				}
		...
		override protected function partRemoved(partName:String, instance:Object):void
		{
			...
			if (instance == navigator)
			{
				...
				// copy proxied values from navigator (if explicitely set) to contentGroupProperties
				
				var newContentGroupProperties:Object = {};
				
				if (BitFlagUtil.isSet(contentGroupProperties as uint, AUTO_LAYOUT_PROPERTY_FLAG))
					newContentGroupProperties.autoLayout = navigator.autoLayout;
				
				if (BitFlagUtil.isSet(contentGroupProperties as uint, LAYOUT_PROPERTY_FLAG))
					newContentGroupProperties.layout = navigator.layout;
				
				contentGroupProperties = newContentGroupProperties;
				
				var myMxmlContent:Array = navigator.getMXMLContent();
				...
				navigator.mxmlContent = null;
				navigator.layout = null;
			}
		}
		...
		public function toArray():Array
		{
			if (navigator)
			{
				return navigator.getMXMLContent();
			}
			else if (_placeHolderGroup)
			{
