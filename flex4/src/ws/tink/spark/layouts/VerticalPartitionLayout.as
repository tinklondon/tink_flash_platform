...
package ws.tink.spark.layouts
{
	 ...
	 */
	public class VerticalPartitionLayout extends LayoutBase
	{
//		include "../core/Version.as";
		...
		private static function sizeLayoutElement(layoutElement:ILayoutElement, 
												  ...
												  rowHeight:Number, numColumns:int):void
		{
...
//			trace( "restrictedWidth", restrictedWidth );
			//TODO edit tink
			newWidth = width / numColumns;
		 ...
		 *  @productversion Flex 4
		 */    
		public function VerticalPartitionLayout():void
		{
			super();
		...
		private function getNextBreak( index:int, breaks:Vector.<int>, total:int ):int
		{
			if( breaks == null ) return total;
			
			if( index < breaks.length ) return breaks[ index ];
//			for each( var i:int in breaks )
//			{
//				if( i > index ) return i;
//			}
		...
		private function getNextPartition( numElements:int, partition:Partition, partitions:Vector.<int> ):Partition
		{
			if( partitions )
			{
				if( partitions.length )
				{
					// If the index is NaN it must be the first loop, set the value to 0
					// if not increment the value
					partition.index = ( isNaN( partition.index ) ) ? 0 : partition.index + 1;
					
					if( partition.index < partitions.length )
					{
						// If there are no items in this partition get the next one
						if( partitions[ partition.index ] < 1 )
						{
							return getNextPartition( numElements, partition, partitions );
						}
						else
						{
							partition.value += partitions[ partition.index ];
							return partition;
						}
					}
					else
					{
						partition.value = numElements;
						return partition;
					}
				}
			}
		...
		{
			var layoutTarget:GroupBase = GroupBase( target );
			var partitionable:IPartitionable = IPartitionable( target );
			var partitions:Vector.<int> = partitionable.partitions;
			var numPartitions:int = ( partitions ) ? partitions.length : 0;
			...
			var excessHeight:Number = distributeHeight(targetWidth, targetHeight, containerWidth, numPartitions + 1 );
			...
			trace( "ggg", partitions );
			var partition:Partition = getNextPartition( count, new Partition(), partitions );
			...
			for (var index:int = 0; index < count; index++)
			{
				...
				if( index >= partition.value )
				{
					partition = getNextPartition( count, partition, partitions );
					
					var x:Number = x0 + ( ( ( ( targetWidth - ( gap * ( numPartitions  ) ) ) / ( numPartitions + 1 ) ) + gap ) * partition.index );
					y = paddingTop;
				}
		...
		private function distributeHeight(width:Number, 
										  height:Number, 
										  restrictedWidth:Number, numColumns:int ):Number
		{
			...
			for (var index:int = 0; index < count; index++)
			{
				...
				{
					//TODO Tink
					sizeLayoutElement(layoutElement, width - ( gap * ( numColumns - 1 ) ), horizontalAlign, 
						restrictedWidth, NaN, variableRowHeight, rh, numColumns);
			...
			if (totalPercentHeight)
			{
				...
				for each (childInfo in childInfoArray)
				{
					...
					//TODO Tink
					sizeLayoutElement(childInfo.layoutElement, width - ( gap * ( numColumns - 1 ) ), horizontalAlign, 
						restrictedWidth, childSize, 
						variableRowHeight, rh, numColumns);
		...
		protected function calculatePartitionIndex(x:Number, y:Number):int
		{
			var layoutTarget:DataGroup = DataGroup( target );
			...
			var targetWidth:Number = Math.max( 0, layoutTarget.width - paddingLeft - paddingRight );
			var useableWidth:Number = Math.max( 0, targetWidth - ( gap * numPartitions ) );
			
			var columnWidth:Number = useableWidth / ( numPartitions + 1 );
			var sideColumn:Number = columnWidth + ( gap / 2 );
			var centerColumn:Number = columnWidth + gap;
			
			var index:int;
			
			if( x < sideColumn + paddingLeft )
			{
				//first column
				return 0;
			}
			else if( x > layoutTarget.width - sideColumn - paddingRight )
			{
				// last column
			...
			{
				return Math.ceil( ( x - sideColumn + paddingLeft ) / centerColumn );
			}
		}
		 ...
		 *  @private 
		 */
		protected function calculateDropIndex2(x:Number, y:Number, partitionIndex:int ):int
		{
			...
			{
				if( i > 0 ) startIndex += partitions[ i - 1 ];
				if( i < numPartitions ) endIndex += partitions[ i ];
			}
			
			if( partitionIndex == 0 ) startIndex = 0;
			if( partitionIndex == numPartitions ) endIndex = count;
			...
			{
				var elementBounds:Rectangle = getElementBounds( i );
				if( !elementBounds ) continue;
				
				if( elementBounds.top - ( gap / 2 ) <= y && y <= elementBounds.bottom + ( gap / 2 ) )
				{
					var centerY:Number = elementBounds.y + elementBounds.height / 2;
					gg = i;//( y < centerY ) ? i : i + 1;
					break;
				}
			}
...
//			if( gg > 1 ) gg = 1;
		...
		override public function calculateDropLocation(dragEvent:DragEvent):DropLocation
		{
			...
			var partitionDropIndex:int = calculatePartitionIndex( dropPoint.x, dropPoint.y );
			var dropIndex:int = calculateDropIndex2( dropPoint.x, dropPoint.y, partitionDropIndex );
			
			if (dropIndex == -1)
				return null;
			
			// Create and fill the drop location info
			var dropLocation:PartitionDropLocation = new PartitionDropLocation();
			dropLocation.dragEvent = dragEvent;
			dropLocation.dropPoint = dropPoint;
			dropLocation.dropIndex = dropIndex;
			dropLocation.partitionDropIndex = partitionDropIndex;
//			trace( "calculateDropLocation" );
			return dropLocation;
		...
		private var _dropIndicatorBounds			: Rectangle;
		private var _dropIndicatorBoundsFailCount	: int = 0;
		...
		{
			var bounds:Rectangle = getElementBounds( dropLocation.dropIndex );
			if( bounds.width == 0 || bounds.height == 0 )
			{
				if( _dropIndicatorBoundsFailCount < 5 ) target.callLater( showDropIndicatorAfterNoBounds );
				_dropIndicatorBoundsFailCount++;
				return null;
			}
			
			_dropIndicatorBoundsFailCount = 0;
			return bounds;
		}
		
		private var _dropLocation	: DropLocation;
		
		private function showDropIndicatorAfterNoBounds():void
		{
			if( _dropLocation ) showDropIndicator( _dropLocation );	
		}
		
		override public function showDropIndicator( dropLocation:DropLocation ):void
		{
			_dropLocation = dropLocation;
			super.showDropIndicator( dropLocation );
		}
		
		override public function hideDropIndicator():void
		{
			_dropLocation = null;
			super.hideDropIndicator();
		}
