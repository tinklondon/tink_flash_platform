...
//	import spark.layouts.HorizontalLayout;
	...
	public class HorizontalSpinnerLayout extends LayoutBase
	{
...
//		 */
//		override public function get firstIndexInView():int
//		{
...
//		 *  @productversion Flex 4
//		 */
//		override public function get lastIndexInView():int
//		{
...
//		}
		...
		{
			return Math.ceil(target.numElements * columnWidth);
		}
		...
		private static const MAX_X_VALUE:Number = int.MAX_VALUE / 20;
		private static const MIN_X_VALUE:Number = int.MIN_VALUE / 20;
		...
		{					
			var preferredHeight:Number = 0;  // max of the elt preferred widths
			...
			if (useVirtualLayout)
			{
				if (typicalLayoutElement)
					preferredHeight = typicalLayoutElement.getPreferredBoundsHeight();
			}
			else
			{
				do
				{
					...
					if (element && element.includeInLayout)
						preferredHeight = Math.max(Math.ceil(element.getPreferredBoundsHeight()), preferredHeight);
					iter.next();
				} 
			...
			var columnsToMeasure:int = getColumsToMeasure(target.numElements);
			
			// Calculate the height by multiplying the number of elements time the row height
			target.measuredWidth = Math.ceil(columnsToMeasure * Math.max(5, columnWidth));
			target.measuredHeight = preferredHeight;
		}
		...
		override public function updateDisplayList(width:Number, height:Number):void
		{
			...
			if (requestedWrapElements && (height > numElements * columnWidth))
			...
			var scrollPosition:Number = horizontalScrollPosition;
			
			var itemIndex:int = Math.floor(scrollPosition / columnWidth);
			var xPos:Number;
			var xPosMax:Number;
			...
			xPos = itemIndex * columnWidth + xOffset;
			
			// Calculate the y position of the bottom of the viewable area
			xPosMax = xPos + columnWidth + width;
			...
			if (numElements > 0)
			{
				do
				{
					...
					if (element && element.includeInLayout)
					{
						...
						element.setLayoutBoundsSize(columnWidth, height);
						element.setLayoutBoundsPosition(xPos, 0);
						
						xPos += columnWidth;
						
						// If we are using virtual layout, only size and position 
						// the visible elements
						if (xPos > xPosMax && !foundLastVisibleElement)
						{
			...
			setColumnCount(numVisibleElements);
			
			trace( "numElements", numElements );
			
			// Set the contentWidth and contentHeight
			target.setContentSize(Math.ceil(numElements * columnWidth), target.height);
		}
...
//				if (((vsp + xOffset + g.getPreferredBoundsHeight()) > MAX_X_VALUE) ||
//					((vsp + xOffset) < MIN_X_VALUE))
//					xOffset = -vsp;
//				g.scrollRect = new Rectangle(hsp, vsp + xOffset, w, h);
				
				// If the verticalScrollPosition exceeds the max/min y value, then the
				// renderers will not be properly positioned. In which case,
				// we offset the y position of the renderers by the verticalScrollPosition
				if (((hsp + xOffset + g.getPreferredBoundsWidth()) > MAX_X_VALUE) ||
					((hsp + xOffset) < MIN_X_VALUE))
					xOffset = -vsp;
				g.scrollRect = new Rectangle(hsp+xOffset, vsp, w, h);
			}
			else
				g.scrollRect = null;
		}
		...
		{		
			return new Rectangle(index * columnWidth, 0, columnWidth, target.measuredHeight); 
		}
		...
		{        
			...
			// Apply the offset        
			var x0:Number = scrollR.left + xOffset;
			var x1:Number = scrollR.right + xOffset - .0001; 
			if (x1 <= x0)
			{
				setIndexInView(-1, -1);
				return;
			...
			if (wrapElements)
			{
				i0 = normalizeItemIndex(Math.floor(x0 / columnWidth));
				i1 = normalizeItemIndex(Math.floor(x1 / columnWidth));
			}
			else
			{
				i0 = Math.min(Math.max(Math.floor(x0 / columnWidth), 0),n);
				i1 = Math.min(Math.max(Math.floor(x1 / columnWidth), 0),n);
			}
			...
			if (wrapElements)
			{
				if (!firstElement || !lastElement || 
					x0 < firstElement.getLayoutBoundsX() || 
					x1 >= (lastElement.getLayoutBoundsX() + lastElement.getLayoutBoundsWidth()))
				{
			...
			{
				if (!firstElement || !lastElement || 
					(x0 < firstElement.getLayoutBoundsX() && firstIndexInView != 0) || 
					(x1 >= (lastElement.getLayoutBoundsX() + lastElement.getLayoutBoundsWidth()) && lastIndexInView != n))
				{
					g.invalidateDisplayList();
				}
			}
		...
		{
			var index:int = Math.floor(position.x / columnWidth); // may be larger than numElements to indicate wrapping
		 ...
		 */ 
		public function getIndexAtHorizontalCenter():int
		{
			var midX:Number = target.getLayoutBoundsWidth() / 2;
			var hsp:Number = wrapElements ? normalizeScrollPosition(horizontalScrollPosition + midX) : horizontalScrollPosition + midX;
			
			return getElementNearestScrollPosition(new Point(hsp, 0), "center"); 
		}
		...
		public function getClosestUnwrappedElementIndex(index:int):int
		{
			if (wrapElements)
			{
				...
				var midHSP:Number = target.getLayoutBoundsWidth() / 2 + horizontalScrollPosition;
				var wrapCount:int = Math.floor(midHSP / totalWidth);
				
				// Get unwrapped middle index
				var centerIndex:int = getElementNearestScrollPosition(new Point(midHSP, 0), "center");
		...
		private function normalizeScrollPosition(hsp:int):int
		{
		 ...
		 *  @return columns to measure based on elements in layout and any requested/min/max rowCount settings. 
		 */
		mx_internal function getColumsToMeasure(numElementsInLayout:int):int
		{
			var columnsToMeasure:int;
			if (requestedColumnCount != -1)
				columnsToMeasure = requestedColumnCount;
